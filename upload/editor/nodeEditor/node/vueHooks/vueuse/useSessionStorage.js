...
export class defineUseSessionStorage extends nodeMeta {
  constructor() {
    super();
    ...
    this.properties = {
      ...
      exported: false,
      declareType: "const",
      outputVar: { id: uid(), isSlot: false, value: `session_${uid().slice(0, 8)}` },
  ...
  onExecute() {
    ...
    if (props.outputVar?.isSlot) {
      declVar = getParamValue(props.outputVar) || `session_${uid().slice(0, 8)}`;
    } else {
      declVar = props.outputVar?.value || `session_${uid().slice(0, 8)}`;
    ...
    if (!key) {
      this.jsCode = `// useSessionStorage: key is required`;
    ...
    // 生成代码
    this.importStr = `import { useSessionStorage } from '@vueuse/core';`;
    this.jsCode = `${declPrefix}${declVar} = useSessionStorage(${key}${initialValueStr}${optionsStr});`;

    this.setOutputData(this.findOutputSlot("outValue"), declVar);
  }

  static get id() { return "vueuse-usesessionstorage-b2c3d4e5-f6a7-8901-bcde-f23456789012"; }
  static get title() { return "useSessionStorage"; }
