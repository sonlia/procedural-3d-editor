    ...
    >
      ...
      <div className="absolute top-8 right-2 z-10 flex items-center gap-1.5">
        ...
        <div className="relative">
          ...
          {uiState.dropdown === "display" && (
            <div className="absolute right-0 top-full mt-1 bg-zinc-900 border border-zinc-700 rounded shadow-lg py-1 min-w-[80px]">
              {["solid", "wireframe", "points", "xray", "normals"].map((m) => (
                <button
                  key={m}
                  onClick={() => {
                    useEditor.getState().setDisplayMode(m);
                    setUiState({ dropdown: null });
                  }}
                  className={`block w-full text-left px-3 py-1 text-[10px] ${
                    displayMode === m ? "text-cyan-400 bg-cyan-950/30" : "text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800"
                  }`}
                >
                  {m}
                </button>
        ...
        <div className="relative">
          <button
            onClick={() => setUiState((s) => ({ ...s, dropdown: s.dropdown === "camera" ? null : "camera" }))}
            className={`px-2 py-1 text-[10px] font-medium rounded border transition-colors flex items-center gap-1 ${
              uiState.dropdown === "camera"
                ? "bg-cyan-500 text-black border-cyan-400"
                : "bg-black/60 text-zinc-400 border-zinc-700/50 hover:text-zinc-200"
            }`}
          >
            {activeCameraId === "default" || !activeCameraId ? "Default Cam" : "Graph Cam"}
            <span className="text-[8px]">▼</span>
          </button>
          {uiState.dropdown === "camera" && (
            <div className="absolute right-0 top-full mt-1 bg-zinc-900 border border-zinc-700 rounded shadow-lg py-1 min-w-[120px]">
              <button
                onClick={() => {
                  useEditor.getState().setActiveCameraId("default");
                  setUiState({ dropdown: null });
                }}
                className={`block w-full text-left px-3 py-1 text-[10px] ${
                  activeCameraId === "default" || !activeCameraId ? "text-cyan-400 bg-cyan-950/30" : "text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800"
                }`}
              >
                Default Camera
              </button>
